...
echo ""
echo -e "Next steps (composition: ${CYAN}${COMPOSITION}${NC}):"
echo ""
echo -e "  ${BOLD}Bootstrap (all binaries from golgiBody depot):${NC}"
echo -e "    1. Clone cellMembrane: ${YELLOW}cd ${ECO_ROOT} && git clone ssh://git@${DNS}:2222/ecoPrimals/cellMembrane.git gardens/cellMembrane${NC}"
echo -e "    2. Clone wateringHole: ${YELLOW}git clone ssh://git@${DNS}:2222/ecoPrimals/wateringHole.git infra/wateringHole${NC}"
echo -e "    3. Build membrane:     ${YELLOW}cd gardens/cellMembrane && cargo build --release${NC}"
echo -e "    4. Bootstrap:          ${YELLOW}./target/release/membrane gate.bootstrap ${GATE_NAME}${NC}"
echo -e "       (fetches genomeBins from ${CYAN}https://depot.primals.eco${NC} for your architecture)"
echo ""
echo -e "The gate is reachable via mesh at ${MESH_IP}. Agents on other gates can"
echo -e "SSH in for fully automated convergence."
